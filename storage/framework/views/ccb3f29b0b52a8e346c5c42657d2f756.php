<?php $__env->startSection('title', 'Detail Verifikasi Pengajuan - ' . $submission->nomor_pengajuan); ?>
...
<div class="space-y-6 animate-fadeIn">
    ...
    <div class="flex items-center space-x-3">
        <a href="<?php echo e(route('admin.pengajuan')); ?>" class="inline-flex items-center text-xs font-bold text-slate-500 hover:text-primary-600">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-7 bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 sm:p-8 space-y-6">
            <div class="border-b border-slate-100 pb-4 flex justify-between items-center">
                <div>
                    ...
                    <h2 class="text-xl font-bold text-secondary-900 font-mono tracking-wide"><?php echo e($submission->nomor_pengajuan); ?></h2>
                </div>
                <!-- Status Badge -->
                <div>
                    <?php if($submission->status === 'Menunggu Verifikasi'): ?>
                    ...
                    <?php elseif($submission->status === 'Perlu Perbaikan'): ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 text-sm">
                <div>
                    ...
                    <p class="text-secondary-800 font-bold mt-1"><?php echo e($submission->nama_pemilik); ?></p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 font-semibold block">Nomor Telepon / WA:</span>
                    <p class="text-primary-700 font-bold mt-1">
                        <a href="https://wa.me/<?php echo e(preg_replace('/[^0-9]/', '', $submission->nomor_telepon)); ?>" target="_blank" class="hover:underline flex items-center space-x-1.5">
                            <span><?php echo e($submission->nomor_telepon); ?></span>
                            <!-- WA Icon -->
                ...
                <div>
                    ...
                    <p class="text-secondary-800 font-bold mt-1"><?php echo e($submission->nama_usaha); ?></p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 font-semibold block">Kategori Usaha:</span>
                    <p class="text-secondary-800 font-bold mt-1"><?php echo e($submission->jenis_usaha); ?></p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 font-semibold block">Nomor Induk Berusaha (NIB):</span>
                    <p class="text-secondary-800 font-mono font-bold mt-1"><?php echo e($submission->nib ?? '-'); ?></p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 font-semibold block">Sertifikat Halal:</span>
                    <p class="text-secondary-800 font-mono font-bold mt-1"><?php echo e($submission->sertifikasi_halal ?? '-'); ?></p>
                </div>
                <div class="sm:col-span-2">
                    <span class="text-xs text-slate-400 font-semibold block">Deskripsi Usaha:</span>
                    <p class="text-secondary-600 leading-relaxed mt-1 bg-slate-50 p-4 rounded-xl border border-slate-100"><?php echo e($submission->deskripsi_usaha); ?></p>
                </div>
                <div>
                    <span class="text-xs text-slate-400 font-semibold block">Desa:</span>
                    <p class="text-secondary-800 font-bold mt-1"><?php echo e($submission->desa); ?></p>
                </div>
                <div class="sm:col-span-2">
                    <span class="text-xs text-slate-400 font-semibold block">Alamat Usaha Lengkap:</span>
                    <p class="text-secondary-600 mt-1"><?php echo e($submission->alamat_lengkap); ?></p>
                </div>
            </div>
            ...
            <div class="space-y-2">
                ...
                <div class="relative rounded-2xl overflow-hidden border border-slate-200 max-h-[350px]">
                    <img src="<?php echo e(asset($submission->foto_usaha)); ?>" alt="Foto Usaha" class="w-full h-full object-cover">
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-5 bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 sm:p-8 space-y-6">
            ...
            <form action="<?php echo e(route('admin.pengajuan.verifikasi', $submission->id)); ?>" method="POST" class="space-y-6">
                <?php echo csrf_field(); ?>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        ...
                        <label class="flex items-center p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="status" value="Menunggu Verifikasi" class="h-4.5 w-4.5 text-primary-600 focus:ring-primary-500" <?php echo e($submission->status === 'Menunggu Verifikasi' ? 'checked' : ''); ?> onchange="toggleNoteField(false)">
                        ...
                        <!-- Perlu Perbaikan -->
                        <label class="flex items-center p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="status" value="Perlu Perbaikan" class="h-4.5 w-4.5 text-primary-600 focus:ring-primary-500" <?php echo e($submission->status === 'Perlu Perbaikan' ? 'checked' : ''); ?> onchange="toggleNoteField(true)">
                        ...
                        <!-- Disetujui -->
                        <label class="flex items-center p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="status" value="Disetujui" class="h-4.5 w-4.5 text-primary-600 focus:ring-primary-500" <?php echo e($submission->status === 'Disetujui' ? 'checked' : ''); ?> onchange="toggleNoteField(false)">
                        ...
                        <!-- Ditolak -->
                        <label class="flex items-center p-3 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-50 transition-colors">
                            <input type="radio" name="status" value="Ditolak" class="h-4.5 w-4.5 text-primary-600 focus:ring-primary-500" <?php echo e($submission->status === 'Ditolak' ? 'checked' : ''); ?> onchange="toggleNoteField(true)">
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\ADVAN\Downloads\sipanda\resources\views/admin/detail-pengajuan.blade.php ENDPATH**/ ?>